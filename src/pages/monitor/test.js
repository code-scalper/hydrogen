const test = {
  P_LqTk: {
    key: "P_LqTk",
    name: "탱크 내부 압력",
    unit: "MPaa",
    description: "탱크 내부 압력",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 1,
  },
  T_LqTk: {
    key: "T_LqTk",
    name: "탱크 내부 온도",
    unit: "℃",
    description: "탱크 내부 온도",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 2,
  },
  m_Sum_LqTk: {
    key: "m_Sum_LqTk",
    name: "탱크 내부 가스의 질량",
    unit: "kg",
    description: "탱크 내부 가스의 질량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 3,
  },
  T_GTkO: {
    key: "T_GTkO",
    name: "",
    unit: "K",
    description: "탱크 내 기상부 온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 4,
  },
  T_ITkO: {
    key: "T_ITkO",
    name: "",
    unit: "K",
    description: "탱크 내 기액 경계면 온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 5,
  },
  T_LTkO: {
    key: "T_LTkO",
    name: "",
    unit: "K",
    description: "탱크 내 액상부 온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 6,
  },
  P_GTkO: {
    key: "P_GTkO",
    name: "",
    unit: "MPaa",
    description: "탱크 내 기상부 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 7,
  },
  P_OpStO_LqTk: {
    key: "P_OpStO_LqTk",
    name: "",
    unit: "MPa",
    description: "압력상승 국면에서 개방 개시 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 8,
  },
  P_ReseatO_LqTk: {
    key: "P_ReseatO_LqTk",
    name: "",
    unit: "MPa",
    description: "압력강하 국면에서 폐쇄 개시 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 9,
  },
  m_BOGO: {
    key: "m_BOGO",
    name: "",
    unit: "kg/s",
    description: "BOG 발생속도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 10,
  },
  m_PSVO_LqTk: {
    key: "m_PSVO_LqTk",
    name: "",
    unit: "kg/s",
    description: "PSV 방출유속",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 11,
  },
  Q_InGO: {
    key: "Q_InGO",
    name: "",
    unit: "kJ/s",
    description: "탱크 내 기상부로 들어가는 열량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 12,
  },
  Q_InLO: {
    key: "Q_InLO",
    name: "",
    unit: "kJ/s",
    description: "탱크 내 액상부로 들어가는 열량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 13,
  },
  Q_InGIO: {
    key: "Q_InGIO",
    name: "",
    unit: "kJ/s",
    description: "탱크 내 기상부에서 기액 경계면으로 들어가는 열량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 14,
  },
  Q_InILO: {
    key: "Q_InILO",
    name: "",
    unit: "kJ/s",
    description: "탱크 내 기액 경계면에서 액상부로 들어가는 열량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 15,
  },
  m_LqPpReal: {
    key: "m_LqPpReal",
    name: "실제 펌핑량",
    unit: "kg/h",
    description: "실재 펌핑량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 16,
  },
  m_LqVapReal: {
    key: "m_LqVapReal",
    name: "기화기 실제 기화량",
    unit: "kg/h",
    description: "Vaporizer 실재 기화량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 17,
  },
  P_Source: {
    key: "P_Source",
    name: "Source의 압력",
    unit: "MPa",
    description: "Source(Electrolysis or Pipe)의 압력",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 18,
  },
  T_SourceC: {
    key: "T_SourceC",
    name: "Source의 온도",
    unit: "℃",
    description: "Source(Electrolysis or Pipe)의 온도",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 19,
  },
  m_Source: {
    key: "m_Source",
    name: "Source의 유량",
    unit: "kg/h",
    description: "Source(Electrolysis or Pipe)의 유량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 20,
  },
  P_Buf1: {
    key: "P_Buf1",
    name: "Tank 압력",
    unit: "MPa",
    description: "Tank 압력",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 21,
  },
  M_BufSum1: {
    key: "M_BufSum1",
    name: "Tank 내 가스 질량",
    unit: "kg",
    description: "Tank 내 가스 질량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 22,
  },
  P_Buf2: {
    key: "P_Buf2",
    name: "Tank 압력",
    unit: "MPa",
    description: "Tank 압력",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 23,
  },
  M_BufSum2: {
    key: "M_BufSum2",
    name: "Tank 내 가스 질량",
    unit: "kg",
    description: "Tank 내 가스 질량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 24,
  },
  P_TT1: {
    key: "P_TT1",
    name: "내부 압력",
    unit: "MPa",
    description: "TT 내부 압력",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 25,
  },
  T_TT1: {
    key: "T_TT1",
    name: "내부 온도",
    unit: "K",
    description: "TT 내부 온도",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 26,
  },
  m_TTSum1: {
    key: "m_TTSum1",
    name: "내부 가스 질량",
    unit: "kg",
    description: "TT 내부 가스 질량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 27,
  },
  P_TT2: {
    key: "P_TT2",
    name: "내부 압력",
    unit: "MPa",
    description: "TT 내부 압력",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 28,
  },
  T_TT2: {
    key: "T_TT2",
    name: "내부 온도",
    unit: "K",
    description: "TT 내부 온도",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 29,
  },
  m_TTSum2: {
    key: "m_TTSum2",
    name: "내부 가스 질량",
    unit: "kg",
    description: "TT 내부 가스 질량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 30,
  },
  m_LCpReal: {
    key: "m_LCpReal",
    name: "압축기 유량",
    unit: "kg/h",
    description: "저압 압축기 유량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 31,
  },
  m_MCp1Real: {
    key: "m_MCp1Real",
    name: "압축기 유량",
    unit: "kg/h",
    description: "중압 압축기1 유량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 32,
  },
  m_MCp2Real: {
    key: "m_MCp2Real",
    name: "압축기 유량",
    unit: "kg/h",
    description: "중압 압축기2 유량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 33,
  },
  m_HCpReal: {
    key: "m_HCpReal",
    name: "압축기 유량",
    unit: "kg/h",
    description: "고압 압축기 유량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 34,
  },
  P_TkL: {
    key: "P_TkL",
    name: "탱크 내부 압력",
    unit: "MPa",
    description: "Tank 내부 압력",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 35,
  },
  T_TkL: {
    key: "T_TkL",
    name: "탱크 내부 온도",
    unit: "℃",
    description: "Tank 내부 온도",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 36,
  },
  m_SumL: {
    key: "m_SumL",
    name: "탱크 내부 가스 질량",
    unit: "kg",
    description: "Tank 내부 가스 질량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 37,
  },
  W_ReqL: {
    key: "W_ReqL",
    name: "PSV 필요분출유량",
    unit: "kg/hr",
    description: "PSV 필요분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 38,
  },
  AaL: {
    key: "AaL",
    name: "PSV 필요분출면적",
    unit: "mm2",
    description: "PSV 필요분출면적",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 39,
  },
  SD_1P_MaxL: {
    key: "SD_1P_MaxL",
    name: "1% 농도 도달 거리",
    unit: "m",
    description: "1% 농도 도달 거리(SD: Safety Distance)",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 40,
  },
  P_TkAccL: {
    key: "P_TkAccL",
    name: "",
    unit: "MPa",
    description: "축적압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 41,
  },
  P_TkOpFullL: {
    key: "P_TkOpFullL",
    name: "",
    unit: "MPa",
    description: "완전개방압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 42,
  },
  P_TkOpSrartL: {
    key: "P_TkOpSrartL",
    name: "",
    unit: "MPa",
    description: "개방개시압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 43,
  },
  P_TkL: {
    key: "P_TkL",
    name: "",
    unit: "MPa",
    description: "탱크(뱅크) 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 44,
  },
  W_Req_kgsL: {
    key: "W_Req_kgsL",
    name: "",
    unit: "kg/hr",
    description: "KGS코드에 따른 PSV  필요분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 45,
  },
  W_OpFullL: {
    key: "W_OpFullL",
    name: "",
    unit: "kg/hr",
    description: "완전개방시 분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 46,
  },
  W_OutL: {
    key: "W_OutL",
    name: "",
    unit: "kg/hr",
    description: "분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 47,
  },
  W_OutL: {
    key: "W_OutL",
    name: "",
    unit: "kg/hr",
    description: "분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 48,
  },
  SD_1PL: {
    key: "SD_1PL",
    name: "",
    unit: "kg/hr",
    description: "1% 농도 도달 거리(SD: Safety Distance)",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 49,
  },
  P_TkM: {
    key: "P_TkM",
    name: "탱크 내부 압력",
    unit: "MPa",
    description: "Tank 내부 압력",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 50,
  },
  T_TkM: {
    key: "T_TkM",
    name: "탱크 내부 온도",
    unit: "℃",
    description: "Tank 내부 온도",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 51,
  },
  m_SumM: {
    key: "m_SumM",
    name: "탱크 내부 가스 질량",
    unit: "kg",
    description: "Tank 내부 가스 질량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 52,
  },
  W_ReqM: {
    key: "W_ReqM",
    name: "PSV 필요분량용량",
    unit: "kg/hr",
    description: "PSV 필요분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 53,
  },
  AaM: {
    key: "AaM",
    name: "PSV 필요분출면적",
    unit: "mm2",
    description: "PSV 필요분출면적",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 54,
  },
  SD_1P_MaxM: {
    key: "SD_1P_MaxM",
    name: "1% 농도 도달 거리",
    unit: "m",
    description: "1% 농도 도달 거리(SD: Safety Distance)",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 55,
  },
  P_TkAccM: {
    key: "P_TkAccM",
    name: "",
    unit: "MPa",
    description: "축적압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 56,
  },
  P_TkOpFullM: {
    key: "P_TkOpFullM",
    name: "",
    unit: "MPa",
    description: "완전개방압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 57,
  },
  P_TkOpSrartM: {
    key: "P_TkOpSrartM",
    name: "",
    unit: "MPa",
    description: "개방개시압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 58,
  },
  P_TkM: {
    key: "P_TkM",
    name: "",
    unit: "MPa",
    description: "탱크(뱅크) 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 59,
  },
  W_Req_kgsM: {
    key: "W_Req_kgsM",
    name: "",
    unit: "kg/hr",
    description: "KGS코드에 따른 PSV  필요분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 60,
  },
  W_OpFullM: {
    key: "W_OpFullM",
    name: "",
    unit: "kg/hr",
    description: "완전개방시 분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 61,
  },
  W_OutM: {
    key: "W_OutM",
    name: "",
    unit: "kg/hr",
    description: "분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 62,
  },
  W_OutM: {
    key: "W_OutM",
    name: "",
    unit: "kg/hr",
    description: "분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 63,
  },
  SD_1PM: {
    key: "SD_1PM",
    name: "",
    unit: "kg/hr",
    description: "1% 농도 도달 거리(SD: Safety Distance)",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 64,
  },
  P_TkH: {
    key: "P_TkH",
    name: "Tank 내부 압력",
    unit: "MPa",
    description: "Tank 내부 압력",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 65,
  },
  T_TkH: {
    key: "T_TkH",
    name: "Tank 내부 온도",
    unit: "℃",
    description: "Tank 내부 온도",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 66,
  },
  m_SumH: {
    key: "m_SumH",
    name: "Tank 내부 가스 질량",
    unit: "kg",
    description: "Tank 내부 가스 질량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 67,
  },
  W_ReqH: {
    key: "W_ReqH",
    name: "PSV 필요분출용량",
    unit: "kg/hr",
    description: "PSV 필요분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 68,
  },
  AaH: {
    key: "AaH",
    name: "PSV 필요분출면적",
    unit: "mm2",
    description: "PSV 필요분출면적",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 69,
  },
  SD_1P_MaxH: {
    key: "SD_1P_MaxH",
    name: "1% 농도 도달 거리",
    unit: "m",
    description: "1% 농도 도달 거리(SD: Safety Distance)",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 70,
  },
  P_TkAccH: {
    key: "P_TkAccH",
    name: "",
    unit: "MPa",
    description: "축적압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 71,
  },
  P_TkOpFullH: {
    key: "P_TkOpFullH",
    name: "",
    unit: "MPa",
    description: "완전개방압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 72,
  },
  P_TkOpSrartH: {
    key: "P_TkOpSrartH",
    name: "",
    unit: "MPa",
    description: "개방개시압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 73,
  },
  P_TkH: {
    key: "P_TkH",
    name: "",
    unit: "MPa",
    description: "탱크(뱅크) 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 74,
  },
  W_Req_kgsH: {
    key: "W_Req_kgsH",
    name: "",
    unit: "kg/hr",
    description: "KGS코드에 따른 PSV  필요분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 75,
  },
  W_OpFullH: {
    key: "W_OpFullH",
    name: "",
    unit: "kg/hr",
    description: "완전개방시 분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 76,
  },
  W_OutH: {
    key: "W_OutH",
    name: "",
    unit: "kg/hr",
    description: "분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 77,
  },
  W_OutH: {
    key: "W_OutH",
    name: "",
    unit: "kg/hr",
    description: "분출유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 78,
  },
  SD_1PH: {
    key: "SD_1PH",
    name: "",
    unit: "kg/hr",
    description: "1% 농도 도달 거리(SD: Safety Distance)",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 79,
  },
  m1301: {
    key: "m1301",
    name: "충전라인 통과 유량",
    unit: "kg/h",
    description: "충전라인 통과 유량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 80,
  },
  m1401: {
    key: "m1401",
    name: "충전라인 통과 유량",
    unit: "kg/h",
    description: "충전라인 통과 유량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 81,
  },
  m1501: {
    key: "m1501",
    name: "충전라인 통과 유량",
    unit: "kg/h",
    description: "충전라인 통과 유량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 82,
  },
  m1302: {
    key: "m1302",
    name: "충전라인 통과 유량",
    unit: "kg/h",
    description: "충전라인 통과 유량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 83,
  },
  m1402: {
    key: "m1402",
    name: "충전라인 통과 유량",
    unit: "kg/h",
    description: "충전라인 통과 유량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 84,
  },
  m1502: {
    key: "m1502",
    name: "충전라인 통과 유량",
    unit: "kg/h",
    description: "충전라인 통과 유량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 85,
  },
  mD1: {
    key: "mD1",
    name: "충전유량",
    unit: "kg/h",
    description: "충전유량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 86,
  },
  mD2: {
    key: "mD2",
    name: "충전유량",
    unit: "kg/h",
    description: "충전유량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 87,
  },
  P_Tk1: {
    key: "P_Tk1",
    name: "탱크 압력",
    unit: "MPa",
    description: "Tank 압력",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 88,
  },
  T_TkC1: {
    key: "T_TkC1",
    name: "탱크 온도",
    unit: "℃",
    description: "Tank 온도",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 89,
  },
  M_Sum1: {
    key: "M_Sum1",
    name: "탱크 내 가스 질량",
    unit: "kg",
    description: "Tank 내 가스 질량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 90,
  },
  SOC_Tk1: {
    key: "SOC_Tk1",
    name: "탱크 내 가스 SOC",
    unit: "%",
    description: "Tank 내 가스 SOC",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 91,
  },
  P_Tk2: {
    key: "P_Tk2",
    name: "탱크 압력",
    unit: "MPa",
    description: "Tank 압력",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 92,
  },
  T_TkC2: {
    key: "T_TkC2",
    name: "탱크 온도",
    unit: "℃",
    description: "Tank 온도",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 93,
  },
  M_Sum2: {
    key: "M_Sum2",
    name: "탱크 내 가스 질량",
    unit: "kg",
    description: "Tank 내 가스 질량",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 94,
  },
  SOC_Tk2: {
    key: "SOC_Tk2",
    name: "탱크 내 가스 SOC",
    unit: "%",
    description: "Tank 내 가스 SOC",
    type: "output",
    displayOnDiagram: true,
    x: 0.1,
    y: 0.1,
    order: 95,
  },
  SOC_Tk1: {
    key: "SOC_Tk1",
    name: "",
    unit: "%",
    description: "Tank1 내 가스 SOC",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 96,
  },
  m_HFP1: {
    key: "m_HFP1",
    name: "",
    unit: "g/s",
    description: "Dispens1의 충전유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 97,
  },
  APRR1: {
    key: "APRR1",
    name: "",
    unit: "MPa/min",
    description: "분당 압력상승률",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 98,
  },
  P_Ba1: {
    key: "P_Ba1",
    name: "",
    unit: "MPa",
    description: "공급 가스압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 99,
  },
  T_Ba1: {
    key: "T_Ba1",
    name: "",
    unit: "℃",
    description: "공급 가스온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 100,
  },
  P_Tk1: {
    key: "P_Tk1",
    name: "",
    unit: "MPa",
    description: "Tank1 내 가스압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 101,
  },
  T_Tk1: {
    key: "T_Tk1",
    name: "",
    unit: "℃",
    description: "Tank1 내 가스온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 102,
  },
  T_Amb: {
    key: "T_Amb",
    name: "",
    unit: "℃",
    description: "대기온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 103,
  },
  T_Ba1: {
    key: "T_Ba1",
    name: "",
    unit: "℃",
    description: "Break Away 전단(충전라인) 인입 온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 104,
  },
  P_Tk_01: {
    key: "P_Tk_01",
    name: "",
    unit: "MPa",
    description: "Vehicle Tank 초기 온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 105,
  },
  T_TkMax1: {
    key: "T_TkMax1",
    name: "",
    unit: "℃",
    description: "Vehicle Tank 최고온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 106,
  },
  t_ElepS1: {
    key: "t_ElepS1",
    name: "",
    unit: "sec",
    description: "초단위 충전시간",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 107,
  },
  V_TkTot1: {
    key: "V_TkTot1",
    name: "Vehicle Tank 총 Volume",
    unit: "m3",
    description: "Vehicle Tank 총 Volume",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 108,
  },
  T_Amb: {
    key: "T_Amb",
    name: "대기온도",
    unit: "℃",
    description: "대기온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 109,
  },
  T_Ba1: {
    key: "T_Ba1",
    name: "Break Away 전단(충전라인) 인입 온도",
    unit: "℃",
    description: "Break Away 전단(충전라인) 인입 온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 110,
  },
  P_Tk_01: {
    key: "P_Tk_01",
    name: "Vehicle Tank 초기 온도",
    unit: "MPa",
    description: "Vehicle Tank 초기 온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 111,
  },
  SOC_Tk1: {
    key: "SOC_Tk1",
    name: "Vehicle Tank SOC",
    unit: "%",
    description: "Vehicle Tank SOC",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 112,
  },
  PRR_av1: {
    key: "PRR_av1",
    name: "Pressure Ramp Rate 평균값",
    unit: "MPa/min",
    description: "Pressure Ramp Rate 평균값",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 113,
  },
  CalcIntv_av1: {
    key: "CalcIntv_av1",
    name: "Calculation Interval 평균값",
    unit: "sec",
    description: "Calculation Interval 평균값",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 114,
  },
  m_HFPMax1: {
    key: "m_HFPMax1",
    name: "충전라인 질량유속 최고값",
    unit: "g/s",
    description: "충전라인 질량유속 최고값",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 115,
  },
  P_Tk1: {
    key: "P_Tk1",
    name: "Vehicle Tank 압력",
    unit: "MPa",
    description: "Vehicle Tank 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 116,
  },
  T_TkMax1: {
    key: "T_TkMax1",
    name: "Vehicle Tank 최고온도",
    unit: "℃",
    description: "Vehicle Tank 최고온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 117,
  },
  t_ElepS1: {
    key: "t_ElepS1",
    name: "초단위 충전시간",
    unit: "sec",
    description: "초단위 충전시간",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 118,
  },
  t_ElepM1: {
    key: "t_ElepM1",
    name: "분단위 충전시간",
    unit: "min",
    description: "분단위 충전시간",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 119,
  },
  SOC_Tk1: {
    key: "SOC_Tk1",
    name: "",
    unit: "%",
    description: "Dispenser1의 Vehicle Tank SOC",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 120,
  },
  m_HFP1: {
    key: "m_HFP1",
    name: "",
    unit: "g/s",
    description: "Dispenser1의 충전라인 질량유속",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 121,
  },
  APRR1: {
    key: "APRR1",
    name: "",
    unit: "MPa/min",
    description: "Dispenser1의 Average Pressure Ramp Rate",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 122,
  },
  P_Ba1: {
    key: "P_Ba1",
    name: "",
    unit: "MPa",
    description: "Dispenser1의 Break Away 입구 인입 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 123,
  },
  P_Tk1: {
    key: "P_Tk1",
    name: "",
    unit: "MPa",
    description: "Dispenser1의 Vehicle Tank의 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 124,
  },
  T_Tk1: {
    key: "T_Tk1",
    name: "",
    unit: "℃",
    description: "Dispenser1의 Vehicle Tank의 온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 125,
  },
  D1Step: {
    key: "D1Step",
    name: "",
    unit: ".",
    description: "Dispenser1의 충전 Step",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 126,
  },
  DeFuel1_LBk: {
    key: "DeFuel1_LBk",
    name: "",
    unit: "kg/h",
    description: "Dispenser1의 LBk 연결 충전라인 질량유속",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 127,
  },
  DeFuel1_MBk: {
    key: "DeFuel1_MBk",
    name: "",
    unit: "kg/h",
    description: "Dispenser1의 MBk 연결 충전라인 질량유속",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 128,
  },
  DeFuel1_HBk: {
    key: "DeFuel1_HBk",
    name: "",
    unit: "kg/h",
    description: "Dispenser1의 HBk 연결 충전라인 질량유속",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 129,
  },
  P_Tk1: {
    key: "P_Tk1",
    name: "",
    unit: "MPa",
    description: "Dispenser1의 Vehicle Tank 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 130,
  },
  SOC_Tk2: {
    key: "SOC_Tk2",
    name: "",
    unit: "%",
    description: "Dispenser2의 Vehicle Tank SOC",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 131,
  },
  m_HFP2: {
    key: "m_HFP2",
    name: "",
    unit: "g/s",
    description: "Dispenser2의 충전라인 질량유속",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 132,
  },
  APRR2: {
    key: "APRR2",
    name: "",
    unit: "MPa/min",
    description: "Dispenser2의 Average Pressure Ramp Rate",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 133,
  },
  P_Ba2: {
    key: "P_Ba2",
    name: "",
    unit: "MPa",
    description: "Dispenser2의 Break Away 입구 인입 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 134,
  },
  P_Tk2: {
    key: "P_Tk2",
    name: "",
    unit: "MPa",
    description: "Dispenser2의 Vehicle Tank의 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 135,
  },
  T_Tk2: {
    key: "T_Tk2",
    name: "",
    unit: "℃",
    description: "Dispenser2의 Vehicle Tank의 온도",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 136,
  },
  D2Step: {
    key: "D2Step",
    name: "",
    unit: ".",
    description: "Dispenser2의 충전 Step",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 137,
  },
  DeFuel2_LBk: {
    key: "DeFuel2_LBk",
    name: "",
    unit: "kg/h",
    description: "Dispenser2의 LBk 연결 충전라인 질량유속",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 138,
  },
  DeFuel2_MBk: {
    key: "DeFuel2_MBk",
    name: "",
    unit: "kg/h",
    description: "Dispenser2의 MBk 연결 충전라인 질량유속",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 139,
  },
  DeFuel2_HBk: {
    key: "DeFuel2_HBk",
    name: "",
    unit: "kg/h",
    description: "Dispenser2의 HBk 연결 충전라인 질량유속",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 140,
  },
  P_Tk2: {
    key: "P_Tk2",
    name: "",
    unit: "MPa",
    description: "Dispenser2의 Vehicle Tank 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 141,
  },
  P_HBk: {
    key: "P_HBk",
    name: "",
    unit: "MPa",
    description: "HBk의 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 142,
  },
  P_MBk: {
    key: "P_MBk",
    name: "",
    unit: "MPa",
    description: "MBk의 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 143,
  },
  P_LBk: {
    key: "P_LBk",
    name: "",
    unit: "MPa",
    description: "LBk의 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 144,
  },
  P_TT1: {
    key: "P_TT1",
    name: "",
    unit: "MPa",
    description: "TT1의 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 145,
  },
  P_TT2: {
    key: "P_TT2",
    name: "",
    unit: "MPa",
    description: "TT2의 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 146,
  },
  P_Buf1: {
    key: "P_Buf1",
    name: "",
    unit: "MPa",
    description: "Buf1의 압력",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 147,
  },
  m_HCpCapa: {
    key: "m_HCpCapa",
    name: "",
    unit: "kg/h",
    description: "고압 압축기 압축용량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 148,
  },
  m_HCpReal: {
    key: "m_HCpReal",
    name: "",
    unit: "kg/h",
    description: "고압 압축기 실재 압축유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 149,
  },
  m_MCp1Capa: {
    key: "m_MCp1Capa",
    name: "",
    unit: "kg/h",
    description: "중압 압축기1 압축용량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 150,
  },
  m_MCp2Capa: {
    key: "m_MCp2Capa",
    name: "",
    unit: "kg/h",
    description: "중압 압축기2 압축용량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 151,
  },
  m_MCp1Real: {
    key: "m_MCp1Real",
    name: "",
    unit: "kg/h",
    description: "중압 압축기 실재 압축유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 152,
  },
  m_LCpCapa: {
    key: "m_LCpCapa",
    name: "",
    unit: "kg/h",
    description: "저압 압축기 압축용량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 153,
  },
  m_LCpReal: {
    key: "m_LCpReal",
    name: "",
    unit: "kg/h",
    description: "저압 압축기 실재 압축유량",
    type: "output",
    displayOnDiagram: false,
    x: 0.1,
    y: 0.1,
    order: 154,
  },
};
